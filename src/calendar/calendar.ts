...
export class Calendar extends TailwindElement() {
  ...
  private nextMonth() {
    this.displayedMonth = Calendar.calculateNextMonth(this.displayedMonth);
  }
  ...
  getHeading() {
    return Calendar.month[this.displayedMonth.getMonth()] + " " + this.displayedMonth.getFullYear();
  }

  private renderDay(date: Date) {
    const formattedDate: string = Calendar.formatDate(date);
    const isThisMonth = this.displayedMonth.getMonth() === date.getMonth();
    const isToday = formattedDate === Calendar.formatDate(this.today);
    const isSelectedDate = formattedDate === Calendar.formatDate(this.selectedDate);
  ...
  private calculateDays() {
    ...
    if (weekdayOfLastDayOfMonth !== 0) {
      ...
      const nextMonth: Date = Calendar.calculateNextMonth(this.displayedMonth);
